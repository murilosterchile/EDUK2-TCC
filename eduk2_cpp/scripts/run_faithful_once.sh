...
while (($#)); do
    case "$1" in
        --data-dir)
            [[ $# -ge 2 ]] || { echo "--data-dir needs a directory" >&2; exit 2; }
            DATA_DIR="$2"
            shift 2
            ;;
        --faithful-bin)
            [[ $# -ge 2 ]] || { echo "--faithful-bin needs a path" >&2; exit 2; }
            FAITHFUL_BIN="$2"
            shift 2
            ;;
        --with-ocaml)
            ...
            ;;
        --ocaml-bin)
            [[ $# -ge 2 ]] || { echo "--ocaml-bin needs a path" >&2; exit 2; }
            OCAML_BIN="$2"
            shift 2
            ;;
        -h|--help)
...
[[ -x "$FAITHFUL_BIN" ]] || { echo "faithful executable not found: $FAITHFUL_BIN" >&2; exit 2; }
...
if ((WITH_OCAML)); then
    ...
    [[ -x "$OCAML_BIN" ]] || { echo "OCaml executable not found: $OCAML_BIN" >&2; exit 2; }
fi

elapsed_seconds() {
    awk -v begin="$1" -v end="$2" 'BEGIN { printf "%.9f", (end - begin) / 1000000000 }'
}

faithful_profit() {
    awk '$1 == "profit" { print $2; found = 1; exit } END { if (!found) exit 1 }' <<< "$1"
}

ocaml_profit() {
    awk '
        /^#The optimal value for the given capacity$/ {
            if (getline && $1 ~ /^-?[0-9]+$/) { print $1; found = 1; exit }
        }
        END { if (!found) exit 1 }
    ' <<< "$1"
}
...
((found)) || { echo "no .ukp files found under: $DATA_DIR" >&2; exit 1; }
