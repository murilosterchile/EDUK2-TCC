...
(* $Id: bandbukp2.ml,v 1.7 2005/05/04 19:30:48 poirriez Exp $ *)
(* bandbukp.ml*)
...
| (i,ni)::s ->begin
   ...
   if profit_smaller limz' !(bound.z) then
     ...
     in (i,(nws,s,nz,ncr))
       ...
       (i,(ws',(i,ni-1)::s,z',cr'))
     in
(*   if !bbnewv && not(Dynefflist.insert_and_improve st {key = wsol';v= z'}) then
     backtrack nb items bound st (wsol',sol',z',cr')
   else*)
     r'
end
  ...
  if weight_smaller cr minweights.(i) || i = lasti then 
    ...
    then
      ...
      if nj = 0 then
        ...
        let nz= add_profits z (mult_int_profit nj pj) in
	let nws = add_weights wsol (mult_int_weight nj wj) in
        if ( !bbnewv &&  not( Dynefflist.insert_and_improve st {key = nws;v= nz})) then begin
	  if !trace then Dynefflist.print st;
          complete lasti items minweights actualz bsol st (wsol,sol,z,cr) j
	  ...
	  if j < lasti then
	    ...
	    if profit_smaller (add_profits nz u) !actualz
	    then (wsol,sol,z,cr)
	    else
...
(*    if !nbbests = -1 then min n 500*)
    if !nbbests = -1 then min n (max 500 (n/100))
    else min !nbbests n
  in 
  nbbests := nbb;
  if !trace then begin
